...
import com.scau.bbs.rest.service.PostsService;
...
/** @Author xhh Create By 2017/8/22 */
...
public class UserController extends BaseController {
  ...
  @PostMapping
  public QuarkResult checkUserName(
    ...
    QuarkResult result =
        restProcessor(
            () -> {
              if (!userService.checkUserName(username)) return QuarkResult.warn("用户名已存在，请重新输入");

              if (!userService.checkUserEmail(email)) return QuarkResult.warn("用户邮箱已存在，请重新输入");
              else userService.createUser(email, username, password);
    ...
    QuarkResult result =
        restProcessor(
            () -> {
              ...
              if (loginUser == null) return QuarkResult.warn("用户邮箱不存在，请重新输入");
  ...
  @PutMapping("/{token}")
  public QuarkResult updateUser(
      @PathVariable("token") String token, String username, String signature, Integer sex) {
    QuarkResult result =
        restProcessor(
            () -> {
  ...
  public QuarkResult updatePassword(
      @PathVariable("token") String token, String newpsd, String oldpsd) {
  ...
  @ApiOperation("登出用户")
  @ApiImplicitParams({@ApiImplicitParam(name = "token", value = "发送给用户的唯一令牌", dataType = "String")})
  @PostMapping("/logout")
  ...
  @ApiImplicitParams({@ApiImplicitParam(name = "id", value = "用户的id", dataType = "int")})
