package com.scau.bbs.chat.handler;

import com.alibaba.fastjson.JSON;
import com.alibaba.fastjson.TypeReference;
import com.scau.bbs.chat.protocol.QuarkChatProtocol;
import com.scau.bbs.chat.protocol.QuarkClientProtocol;
import com.scau.bbs.chat.service.ChannelManager;
import com.scau.bbs.chat.utils.NettyUtil;
...
/**
 * @Author : xhh @Date : Create in 10:00 2019/1/12 @Email : xhhscau2015@163.com
 *
 * <p>WebSocket握手/用户认证 Handler
 */
...
public class UserAuthHandler extends SimpleChannelInboundHandler {
  ...
  private void handleHttpRequest(ChannelHandlerContext ctx, FullHttpRequest request) {
    ...
    WebSocketServerHandshakerFactory handshakerFactory =
        new WebSocketServerHandshakerFactory(WEBSOCKET_URL, null, true);
  ...
  private void handleWebSocket(ChannelHandlerContext ctx, WebSocketFrame frame) {
    ...
    if (!(frame instanceof TextWebSocketFrame)) {
      throw new UnsupportedOperationException(
          frame.getClass().getName() + "frame type not supported!!!");
    }
    String message = ((TextWebSocketFrame) frame).text();
    QuarkClientProtocol clientProto =
        JSON.parseObject(message, new TypeReference<QuarkClientProtocol>() {});
    ...
    switch (type) {
      ...
      case AUTH_REQUEST_CODE:
        ...
        if (isSuccess) manager.broadMessage(QuarkChatProtocol.buildSysUserInfo(manager.getUsers()));
        return;
